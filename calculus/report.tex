...
The pattern matching on code from the new macro system of Scala 3 is modelled by a calculus called \calculus. We present a mechanized proof of soundness of the calculus in Coq .
%
...
\todo[inline]{Maybe it's easier to see the type checking problem if you list the type signature of "traverse" and "f", as well as the case for "PVar" or "MatchVar" in traverse.}
...
\todo[inline]{This section is difficult to understand and the readers are easy to get lost. Try to make it more understandable by showing the relevant Coq code if it's clean or by presenting the insight/tricks using simple examples.

Abstract insights/tricks and present them using simple examples without specifics of the project is a very good practice in writing.

Otherwise, interested readers will just check Coq source code to understand more.
}
...
\todo[inline]{iterative}
% We have approached the proofs by first trying to prove a subset of the calculus corresponding to STLC, as a warm-up. Then we have added quotes, splices and lift and updated the proofs. Later we added the pattern matching and at the very end the fixpoint operator. Adding the features step-by-step required rewriting a substantial part of the proofs and sometimes even reformulating some basic definitions. But it also helped to keep the scope more manageable by dividing the proving effort into smaller pieces. The progressive approach also helped me improve my mechanization skills gradually (so in the last proof I was able to use techniques that I wouldn't understand without enough mechanization at the beginning) and better understand the paper proofs that I've been basing on.

\todo[inline]{generalizing}
%\subsection{Generalizing quantifiers}
...
\todo[inline]{Fix the QPM citation}
